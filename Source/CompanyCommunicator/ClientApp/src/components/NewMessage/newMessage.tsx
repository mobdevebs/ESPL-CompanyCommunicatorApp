...
import { Button, Loader, Dropdown, Text, Flex, Input, TextArea, RadioGroup, Checkbox, Datepicker, FormDropdown } from '@fluentui/react-northstar'
...
class NewMessage extends React.Component<INewMessageProps, formState> {
    ...
    private handleImagePDFSelection() {
        ...
        if (file) { //if we have a file
            ...
            if (this.state.templateType === this.localize("ImageUpload")) {  //for image upload
                Resizer.imageFileResizer(file, 400, 400, 'JPEG', 80, 0,
                    uri => {
                        if (uri.toString().length < 32768) {
                            console.log("image", uri.toString().length)
                ...
                // reader.onload = () => {
                //     if (reader.result) {  // create base64 link of pdf
                //         console.log("pdf 1", reader.result.toString().length)
                //         if (reader.result.toString().length < 98305) {
                //             console.log("pdf 2", reader.result.toString().length)
                //             this.setState({
                //                 imageLink: reader.result
                //             }, () => {
                // const newFile = this.dataURLtoFile(this.state.imageLink, "UploadedPdf.pdf") //convert base64 to file
                ...
                if (file.size < 15728640) {
                    ...
                    reader.onload = () => {
                        if (reader.result) {
                            this.setState({
                                emailBodyContent: reader.result
                            })
        ...
        try {
            ...
            this.setState({
                ...
                if (this.state.templateType === this.localize("ImageUpload")) {
                    ...
                    if (draftMessageDetail.imageLink !== "") {
                        // setCardFileNameTitleEmailTemplate(this.card, draftMessageDetail.emailTitle)
                ...
                this.setState({
                    ...
                    sisterTenantOptionSelected: draftMessageDetail.sisterTenant,
                    loader: false,
                    card: this.card,
                        ...
                        {(this.state.templateType === this.localize("ImageUpload") || this.state.templateType === this.localize("PDFUpload")) ? <div className="taskModule">
                            <Flex column className="formContainer" vAlign="stretch" gap="gap.small" styles={{ background: "white" }}>
                                        ...
                                        <Flex column className="formContentContainer">
                                            ...
                                            <Flex gap="gap.smaller" vAlign="end" className="inputField">
                                                ...
                                                /> :
                                                    // <Input
                                                    //     value={this.state.imageLink}
                                                    //     label={this.localize("PDFURL")}
                                                    //     placeholder={this.localize("PDFURLPlaceHolder")}
                                                    //     onChange={this.onImageLinkChanged}
                                                    //     error={!(this.state.errorImageUrlMessage === "")}
                                                    //     autoComplete="off"
                                                    //     fluid
                                                ...
                                                {(this.state.templateType === this.localize("ImageUpload")) ? <input type="file" accept=".png, .jpg, .jpeg, .gif, .CR2, .svg, .tiff, .nef"
                                                    style={{ display: 'none' }}
                                ...
                                <Flex className="footerContainer" vAlign="end" hAlign="end">
                                    <Button content={this.localize("Back")} onClick={() => this.props.history.push(`/customNewmessage`)} secondary />
                            ...
                            (this.state.templateType === this.localize("Q&AUpload")) ? <div className="taskModule">
                                <Flex column className="formContainer" vAlign="stretch" gap="gap.small" styles={{ background: "white" }}>
                                            ...
                                            <Flex column className="formContentContainer">
                                                ...
                                                <div className="textArea">
                                                    <Flex gap="gap.large" vAlign="end" styles={{ marginTop: "10px" }}>
                                                        <Flex.Item push >
                                                            <Button circular size="small" disabled={(this.state.questionAnswer.length == 6) || !(this.state.addQuestionError === "")} icon={<AddIcon />} title={this.localize("Add")} onClick={this.addQuestionAnswer.bind(this)} />
                                                        </Flex.Item>
                                                    </Flex>
                                                </div>
                                    ...
                                    <Flex className="footerContainer" vAlign="end" hAlign="end">
                                        {this.state.backButtonShow && <Button content={this.localize("Back")} onClick={() => this.props.history.push(`/customNewmessage`)} secondary />}
                                        <Flex className="buttonContainer">
                                            <Button content={this.localize("Next")} disabled={this.isNextBtnDisabled()} id="saveBtn" onClick={this.onNext} primary />
                                        </Flex>
                                    </Flex>
    ...
    private isNextBtnDisabled = () => {
        ...
        if (this.state.templateType === this.localize("Q&AUpload")) {
            return !(title && (this.state.errorButtonUrlMessage === "") && (this.state.addQuestionError === ""));
        }
        else {
    ...
    private onSave = () => {
        ...
        const draftMessage: IDraftMessage = {
            ...
            EmailBody: this.state.emailBodyContent ? this.state.emailBodyContent.replace(/\r\n\t|\n|\r|\t/gm," ") :"" ,
    ...
    private onImageLinkChanged = (event: any) => {
        ...
        //     setCardBtnsPDFUpload(this.card, this.state.values);
        //     setCardImageLinkPDFUpload(this.card, pdfImgUrl);
        //     if ((event.target.value.toLowerCase().startsWith("https://")) || (event.target.value.toLowerCase().startsWith("http://"))) {
        //         let pdfLink = "[View PDF](" + JSON.stringify(event.target.value.toLowerCase()) + ")"
        //         setCardPdfNamePDFUpload(this.card, pdfLink)
        //     }
        //     // else {
        //     //     const newFile = this.dataURLtoFile(event.target.value, "UploadedPdf.pdf")
        //     //     let pdfLink = "[View PDF](" + URL.createObjectURL(newFile).toString() + ")"
        //     //     setCardPdfNamePDFUpload(this.card, pdfLink)
        //     // }
        // }
        this.setState({
    ...
    private addQuestionAnswer() {
        ...
        console.log("question dropdown", mcqCount.length);
            ...
            if (this.state.questionAnswer.length > 0) {
                if (this.state.questionAnswer[lastIndex].question !== "") {
                    if (this.state.questionAnswer[lastIndex].questionType === this.localize("MCQ")) {
                        // if (this.state.questionAnswer[lastIndex].answer.length === 4) {
    ...
    handleChangeMCQAnswer = (event: any, i: any, j: any) => {
        let questionAnswer = [...this.state.questionAnswer];
        this.setState({
            mcqAnswer: { ...this.state.mcqAnswer, [j]: event },
            addQuestionError: ""
        }, () => {

            // const mcqAnswerArray = Object.values(this.state.mcqAnswer)
            questionAnswer[i].answer = this.state.mcqAnswer
            this.setState({ questionAnswer }, () => {
