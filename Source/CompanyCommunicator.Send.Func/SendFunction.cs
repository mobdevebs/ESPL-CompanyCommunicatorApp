            ...
            try
            {
               ...
               messageContent.NotificationId);
                if (notification.TemplateType.ToUpper() == "UPLOAD EMAIL TEMPLATE" && (notification.SendTypeId == "2" || notification.SendTypeId == "3"))
                {
                    ...
                    var sentResult = await this.usersService.SendMailToUserAsync(notification.CreatedBy, notification.Title, user.Mail, notification.EmailBody, true);

                    log.LogInformation($"Mail sent status :{sentResult}");
                }
                return true;
            }
            catch (Exception e)
